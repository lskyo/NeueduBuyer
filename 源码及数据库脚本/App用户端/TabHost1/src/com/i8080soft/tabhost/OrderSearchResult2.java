...
import com.i8080soft.tabhost.OrderSearchResult1.UIAsyncTask;
...
public class OrderSearchResult2 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.order_rearch_result2);
		

		list = new ArrayList<OrderData>();
		order_list_result = (ListView) findViewById(R.id.orderList2);
		handler.execute();
			...
			protected void onPostExecute(List<OrderData> result) {
				...
				if (result != null) {
					for (int i = 0; i < result.size(); i++) {
						if((result.get(i).getState()).equals("1")){
					...
					orderSearchAdapter = new OrderSearchAdapter(
							OrderSearchResult2.this, R.layout.reservation_item,
							list);
