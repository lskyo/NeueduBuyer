...
public class OrderSearchResult4 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.order_rearch_result4);
		
		list = new ArrayList<OrderData>();
		order_list_result = (ListView) findViewById(R.id.orderList4);
		handler.execute();
				...
				protected void onPostExecute(List<OrderData> result) {
					...
					if (result != null) {
						for (int i = 0; i < result.size(); i++) {
							if(((result.get(i).getState()).equals("1"))||((result.get(i).getState()).equals("2"))){
						...
						orderSearchAdapter = new OrderSearchAdapter(
								OrderSearchResult4.this, R.layout.reservation_item,
								list);
